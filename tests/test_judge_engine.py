...
from personaforge.backend.app.judge.evaluator import JudgeEngine, EvaluationFailure, LLMFailures
...
async def test_judge_rule_based_looping(judge_engine):
    ...
    judge_engine.llm.get_structured_completion = AsyncMock(return_value=Mock(failures=[]))
...
async def test_judge_rule_based_latency(judge_engine):
    ...
    result = await judge_engine.evaluate_conversation(uuid.uuid4(), history, metadata=metadata)
