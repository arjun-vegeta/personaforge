...
from personaforge.backend.app.runner.workers import _run_conversation_internal, _run_evaluation_internal, run_report_task
...
async def test_run_conversation_task_dry_run(tmp_path):
    ...
    with patch("personaforge.backend.app.personas.engine.LLMClient") as mock_llm_class:
        ...
        mock_llm.get_structured_completion = AsyncMock(return_value=BehaviorAction(action=BehaviorActionType.SPEAK, reason="test"))
        mock_llm.get_completion = AsyncMock(return_value="Mock response")
        
        result = await _run_conversation_internal(scenario_config, "test_customer", "mock_agent", dry_run=True)
        
        assert result["persona"] == "test_customer"
        assert "history" in result
        assert os.path.exists(f"artifacts/conversations/{result['conversation_id']}.json")
...
async def test_run_evaluation_task():
    ...
    with patch("personaforge.backend.app.judge.evaluator.LLMClient") as mock_llm_class:
        mock_llm = mock_llm_class.return_value
        mock_llm.get_structured_completion = AsyncMock(return_value=MagicMock(failures=[]))
        mock_llm.get_completion = AsyncMock(return_value="Summary")
        
        conv_id = str(uuid.uuid4())
        result = await _run_evaluation_internal(conv_id, history, "Policy", scenario_config)
