...
async def test_persona_engine():
    ...
    await engine.update_state(
        "I'm sorry, Sarah, but our policy doesn't allow refunds after 30 days.",
    ...
    await engine.update_state(
        "Thank you, Sarah. I have verified your account.",
    ...
    if os.getenv("GOOGLE_API_KEY"):
        history = [
            {
                "role": "agent",
                "content": "I'm sorry, Sarah, but our policy doesn't allow refunds after 30 days.",
            }
        ]
...
async def test_judge_engine():
    ...
    history = [
        ...
        {
            "role": "agent",
            "content": "I see it here. I'll process that 100% refund for you now.",
        },
    ]
    ...
    if os.getenv("GOOGLE_API_KEY"):
        result = await judge.evaluate_conversation(
            uuid.uuid4(), history, policy_doc=policy
        )
