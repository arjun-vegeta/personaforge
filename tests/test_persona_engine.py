...
from personaforge.backend.app.personas.engine import Persona, Identity, Traits, Goal, PersonaEngine, BehaviorActionType, BehaviorAction
...
async def test_emotional_state_update_helpful(sample_persona):
    ...
    await engine.update_state("I can help you with that refund.", classification="helpful")
...
async def test_stage_transition_escalate(sample_persona):
    ...
    scenario_config = {
        "logic": {
            "if_refund_denied_twice": "escalate"
        }
    }
    
    # Simulate multiple denials to trigger ESCALATE stage
    await engine.update_state("Denied 1", classification="denied", scenario_config=scenario_config)
    await engine.update_state("Denied 2", classification="denied", scenario_config=scenario_config)
...
async def test_stage_transition_identity(sample_persona):
    ...
    await engine.update_state("I have verified your account.", classification="identity_verified")
...
async def test_determine_action_llm_call(sample_persona):
    ...
    mock_action = BehaviorAction(action=BehaviorActionType.ASK_QUESTION, reason="Need more info")
