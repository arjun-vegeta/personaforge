...
class EmotionalState(BaseModel):
    ...
    def update(self, classification: str, traits: Traits):
        ...
        if classification == "helpful":
            ...
            frustration_inc = 0.3 + (traits.aggressiveness * 0.2) - (traits.patience * 0.1)
...
class Persona(BaseModel):
    ...
    current_stage: str = "GREETING" # GREETING, VERIFY_IDENTITY, CORE_GOAL, NEGOTIATE, ESCALATE, EXIT
...
Pay attention to interrupt_probability and topic_shift_probability to determine how erratic or aggressive you should be."""
        ...
        return await self.llm.get_structured_completion(
            messages=messages,
            response_format=BehaviorAction,
            temperature=0.0
        )

    async def generate_utterance(self, action: BehaviorAction, conversation_history: List[Dict[str, str]]) -> str:
