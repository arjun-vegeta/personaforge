    ...
    async def run(self):
        ...
        if self.db_session:
            db_conv = DBConversation(
                ...
                agent_id=uuid.UUID(self.agent_id) if "-" in self.agent_id else None, # Simplified
                status="active"
            )
    ...
    async def handle_event(self, event: Dict[str, Any]):
        ...
        if event_type == "agent_response":
            ...
            if content:
                ...
                await self.persona_engine.update_state(content)
                
                # Determine next action
                action = await self.persona_engine.determine_action(self.history)
