...
async def _run_conversation_internal(
    scenario_config, persona_name, agent_id, dry_run=False
):
        ...
        class MockProvider(VoiceAgentProvider):
            ...
            async def receive_events(self):
                ...
                yield {
                    "type": "agent_response",
                    "agent_response": {
                        "content": "I understand you are looking for a refund."
                    },
                }
                await asyncio.sleep(0.1)
                yield {
                    "type": "agent_response",
                    "agent_response": {
                        "content": "I have checked our records and unfortunately, I cannot process a refund at this time."
                    },
                }
...
def run_conversation_task(scenario_config, persona_name, agent_id, dry_run=False):
    ...
    return asyncio.run(
        _run_conversation_internal(scenario_config, persona_name, agent_id, dry_run)
    )


async def _run_evaluation_internal(
    conversation_id, history, policy_doc, scenario_config
):
...
def run_evaluation_task(conversation_id, history, policy_doc, scenario_config):
    ...
    return asyncio.run(
        _run_evaluation_internal(conversation_id, history, policy_doc, scenario_config)
    )
