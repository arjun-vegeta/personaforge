    ...
    try:
        ...
        if not os.path.exists("personaforge.yaml"):
            raise FileNotFoundError(
                "personaforge.yaml not found. Run 'personaforge init' first."
            )
        ...
        if agent_id == "YOUR_AGENT_ID" and not dry_run:
            raise ValueError(
                "Please set agent_id in personaforge.yaml or scenario file."
            )

        personas_to_run = (
            personas_override if personas_override else scenario.get("personas", [])
        )
        concurrency = concurrency_override or config.get("execution", {}).get(
            "concurrency", 5
        )
        total_conversations = count_override or config.get("execution", {}).get(
            "conversations", 1
        )
                    ...
                    class MockPersonaEngine(PersonaEngine):
                        ...
                        async def determine_action(self, *args, **kwargs):
                            return BehaviorAction(
                                action=BehaviorActionType.SPEAK, reason="Dry run"
                            )
                    ...
                    class MockProvider(VoiceAgentProvider):
                        ...
                        async def receive_events(self):
                            ...
                            yield {
                                "type": "agent_response",
                                "agent_response": {
                                    "content": "I'm sorry to hear that. I can offer a 30% refund."
                                },
                            }
        ...
        for runner in runners:
            ...
            metadata = {
                "avg_latency": sum(runner.total_latencies) / len(runner.total_latencies)
                if runner.total_latencies
                else 0,
            ...
            tts_chars = sum(
                len(m["content"]) for m in runner.history if m["role"] == "agent"
            )
            stt_chars = sum(
                len(m["content"]) for m in runner.history if m["role"] == "customer"
            )
            ...
            if db_session:
                ...
                if db_conv:
                    ...
                    db_conv.interruption_recovery_count = metadata[
                        "interruption_recovery_count"
                    ]
                    ...
                    db_eval = DBEvaluation(
                        ...
                        hallucination_detected=any(
                            f.category == "hallucination" for f in eval_result.failures
                        ),
                        escalation_failure=any(
                            f.category == "escalation" for f in eval_result.failures
                        ),
                        completion_failure=any(
                            f.category == "completion" for f in eval_result.failures
                        ),
        ...
        for res in results:
            with open(
                f"artifacts/conversations/{res['conversation_id']}.json", "w"
            ) as f:
                json.dump(
                    {
...
@click.option(
    "--dry-run", is_flag=True, help="Simulate execution without calling external APIs"
)
    ...
    async def run_internal():
        ...
        try:
            ...
            async for session in get_session():
                ...
                test_run = TestRun(
                    scenario_name=scenario_data.get("name", "Unknown"), status="active"
                )
...
def compare(report1, report2):
    ...
    def get_stats(results):
        ...
        return {
            "total": total,
            "pass_rate": (
                len([r for r in results if r["evaluation"]["pass_status"]])
                / total
                * 100
            )
            if total > 0
    ...
    click.echo(
        f"{'Pass Rate':20} | {s1['pass_rate']:9.1f}% | {s2['pass_rate']:9.1f}% | {pass_change:+.1f}%"
    )
...
def ci(scenario):
    ...
    try:
        ...
        results = asyncio.run(
            execute_run_logic(
                scenario, dry_run=os.getenv("PERSONAFORGE_CI_DRY_RUN") == "true"
            )
        )
    except Exception as e:
        click.echo(f"Error: {e}")
    ...
    total = len(results)
    pass_rate = (
        (len([r for r in results if r["evaluation"]["pass_status"]]) / total * 100)
        if total > 0
        else 0
    )
    ...
    hallucination_rate = (
        (failure_counts.get("hallucination", 0) / total * 100) if total > 0 else 0
    )
    escalation_failures = failure_counts.get("escalation", 0)
    escalation_accuracy = (
        ((total - escalation_failures) / total * 100) if total > 0 else 100
    )
    ...
    if pass_rate < comp_thresh:
        click.echo(
            f"❌ FAILED: Pass rate {pass_rate:.1f}% below threshold {comp_thresh}%"
        )
        failed_ci = True
    else:
    ...
    if hallucination_rate > hall_thresh:
        click.echo(
            f"❌ FAILED: Hallucination rate {hallucination_rate:.1f}% above threshold {hall_thresh}%"
        )
        failed_ci = True
    else:
        click.echo(
            f"✅ Hallucination Rate: {hallucination_rate:.1f}% (Threshold: {hall_thresh}%)"
        )

    esc_thresh = eval_config.get("escalation_threshold", 90)
    if escalation_accuracy < esc_thresh:
        click.echo(
            f"❌ FAILED: Escalation accuracy {escalation_accuracy:.1f}% below threshold {esc_thresh}%"
        )
        failed_ci = True
    else:
        click.echo(
            f"✅ Escalation Accuracy: {escalation_accuracy:.1f}% (Threshold: {esc_thresh}%)"
        )
...
@click.option(
    "--queue",
    default="all",
    help="Queue to listen to (conversation, evaluation, report, or all)",
)
def worker(queue):
