    ...
    return [
        {
            ...
            "status": run.status,
            "pass_rate": (run.passed_conversations / run.total_conversations * 100)
            if run.total_conversations > 0
            else 0,
        ...
        enriched_conversations.append(
            {
                ...
                "status": "passed"
                if evaluation and evaluation.result.get("pass_status", False)
                else "failed",
                "total_cost": conv.total_cost,
                "passed": evaluation.result.get("pass_status", False)
                if evaluation
                else False,
...
async def get_conversation(
    ...
    eval_statement = select(Evaluation).where(
    ...
    pass_rate_over_time = [
        {
            ...
            "rate": (run.passed_conversations / run.total_conversations * 100)
            if run.total_conversations > 0
            else 0,
        }
...
async def get_agent_health(session: AsyncSession = Depends(get_session)):
    ...
    for i, run in enumerate(reversed(runs)):
        ...
        eval_statement = (
            select(Evaluation)
            .join(Conversation)
            .where(Conversation.test_run_id == run.id)
        )
        eval_results = await session.execute(eval_statement)
        ...
        escalation_rate = (
            ((total - escalation_failure_count) / total * 100) if total > 0 else 100
        )

        # Avg Latency for the run
        avg_latency = (
            run.total_cost / total if total > 0 else 0
        )  # This is a placeholder for cost, let's use actual latency if we have it aggregated
        # Better: calculate from conversations
        latency_statement = select(func.avg(Conversation.avg_latency)).where(
            Conversation.test_run_id == run.id
        )
    ...
    for row in results:
        ...
        eval_statement = (
            ...
            .where(
                ...
                Evaluation.severity
                == "low",  # Simplified: "low" severity means it passed
            )
        )
