...
class JudgeEngine:
    ...
    async def evaluate_conversation(
        ...
        llm_failures = await self._run_llm_evaluations(
            history, policy_doc, scenario_config
        )
        failures.extend(llm_failures)

        pass_status = (
            len([f for f in failures if f.severity in ["high", "critical"]]) == 0
        )
...
Output the failures in the requested JSON format."""
        ...
        try:
            result = await self.llm.get_structured_completion(
                messages=messages, response_format=LLMFailures, temperature=0.0
            )
